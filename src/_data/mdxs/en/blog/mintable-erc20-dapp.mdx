...
In Hardhat we can deploy a contract in a `localhost` network. It is needed to start a local node and deploy the contract. In this app `npm run node` and `npm run compile-share-deploy`, respectively.
