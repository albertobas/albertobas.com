---
field: 'defi'
title: 'End-to-end guide to create an ERC20 token specific dapp'
description: 'Decentralized application to operate with a mintable ERC20 token.'
introduction: "<p>In this post I'll be developing a decentralized application to operate with a mintable ERC20 token.</p><p>The purpose is then to understand the end-to-end process of writing a smart contract in Solidity and developing a specific front-end to interact with this contract.</p>"
...
I use [npm 7 workspaces](https://docs.npmjs.com/cli/v7/using-npm/workspaces 'npm 7 workspaces') to create a monorepo in order to simplify our development environment.
...
Now let's create the workspaces folders:
...
We are ready to install the dependencies. I'll start with the Hardhat environment:
...
And now let's install the `react-app` dependencies:
...
Lastly, I will add some scripts that will be handy:
...
I use a hardhat plugin for integration with [Waffle](https://hardhat.org/plugins/nomiclabs-hardhat-waffle.html) and another for [Ethers.js](https://hardhat.org/plugins/nomiclabs-hardhat-ethers.html) to write the required tests with [Mocha](https://mochajs.org/) alongside [Chai](https://www.chaijs.com/).
...
Before running the script we need to create a hardhat configuration file.
...
Now let's run the script `test-hardhat`:
...
As our front-end application is in a different workspace we'll have to write a copy of this files in the `react-app` workspace in order to be able to reach them:
...
This is easily accomplished by adding a simple script under the `tasks` folder.

> To simplify the process, a pre-specified amount of 1000000 tokens is set.
...
Lastly, we just need to import this file in our hardhat configuration:
...
In Hardhat we can deploy a contract in a `localhost` network. We need to start a local node and deploy, in our case `npm run node` and `npm run compile-share-deploy` respectively.

The former command will ensure our React app has the current contract address, typings and ABI after compiling the solidity file and deploying it in the `localhost` network.
...
In order for our dapp to work it is needed that a browser crypto wallet is installed and injects the `ethereum` property to the `window`.
...
I keep the account address and both contracts in [`ContractContext`](https://www.github.com/albertobas/mintable-erc20-dapp/blob/main/react-app/src/components/context/ContractContext.tsx) which will provide us with them in any of the components that are descendants to `ContractProvider`.

Once the data in the context is defined, the components [`Balance`](https://www.github.com/albertobas/mintable-erc20-dapp/blob/main/react-app/src/components/Balance.tsx), [`TransferFrom`](https://www.github.com/albertobas/mintable-erc20-dapp/blob/main/react-app/src/components/TransferFrom.tsx) and [`Panels`](https://www.github.com/albertobas/mintable-erc20-dapp/blob/main/react-app/src/components/Panels.tsx) will be rendered.
...
I set listeners inside `useEffect`s in custom hooks to keep all three main components up to date with the state in the blockchain (also `TransferFrom`, since the input data is validated using the `Formik` library).
...
Now that we have all the logic implemented, we'd only need to run `npm run start-app`.
