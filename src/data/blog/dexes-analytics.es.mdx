...
description: 'Introducción a la obtención y representación de datos de DEXes utilizando TheGraph and React.js.'
introduction: "<p>Los exchanges descentralizados (DEXes) generan enormes cantidades de datos que son interesantes de leer pero difíciles de recopilar y representar.</p><p>Afortunadamente para nosotros, [TheGraph](https://thegraph.com/ 'Ir a la web de TheGraph') viene a nuestro rescate para facilitar una solución y hacer posible recuperar la cantidad mínima requerida de datos que se necesitan de una manera simple usando [GraphQL](https://graphql.org/ 'Ir a la web de GraphQL'). Veamos.</p>"
...
En esta publicación intentaré explicar cómo crear una aplicación creada con [Vite.js](https://vitejs.dev 'Ir a la web de Vite.js') que consulte los _subgraphs_ de Uniswap v2 y de v3 en TheGraph utilizando un cliente de GraphQL básico, procese los datos, los almacene en un estado global y los represente utilizando componentes de React.js.
...
De esta manera, por ejemplo, la transición a una interfaz de usuario diferente se lograría editando solo las carpetas _ui_.
...
Por ejemplo, al navegar a [Uniswap v3 overview](https:// 'Uniswap V3 overview'), se renderizará un componente React.js y el _hook_ `useUniswapV3PoolsTokens` activará cuatro casos de uso o _interactors_:
...
Cada _interactor_ llama a un método de un repositorio, que se implementa en una clase en las fuentes de datos. Este método consultará un _subgraph_ en TheGraph con un cliente GraphQL y devolverá un objeto tipo JSON que, posteriormente, se adaptará a una interfaz común a todos los protocolos.
...
Cada _feature_, o protocolo soportado, reúne varias _features_. Siguiendo el ejemplo mencionado anteriormente, echemos un vistazo a cada carpeta de la _feature_ implementada Uniswap v3.
...
Este es el caso de uso para generar una entidad `Tokens`. Básicamente obtiene un _endpoint_ y un `Blocks` y los pasa al método `getByAddresses` de `TokensUniswapV3Repository`.
...
La única fuente de datos en tokens es una clase que implementa `TokensUniswapV3Repository`, que solo requiere el método `getByAddresses`.
...
Un `Blocks` se pasa a `getByAddresses` porque necesitamos consultar cuatro marcas de tiempo diferentes, esto se logra pasando el número de bloque en la consulta.
