...
oGImage: { name: mintable-erc-20-dapp.es.png, width: 1200, height: 630 }
---
...
El nombre de este _workspace_ es el mismo que el nombre del paquete `hardhat` que ya hemos instalado, así que, para importar los archivos que necesitamos en `react-app` podemos o bien renombrar este _workspace_ para evitar cualquier conflicto de nombres e importarlos desde este _workspace_, o, como mostramos a continuación, dejarlo así y escribir un _script_ simple para leer los archivos que necesitamos y escribirlos en el _workspace_ `react-app` para importarlos desde ahí:
